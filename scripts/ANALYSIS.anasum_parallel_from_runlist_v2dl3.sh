...
# begin help message
echo "
ANASUM parallel data analysis: submit jobs using a simple run list, create v2dl3 script.

ANALYSIS.anasum_parallel_from_runlist_v2dl3.sh <run list> <output directory> <cut set> <background model> [run parameter file] [mscw directory] [sim type] \
...
for RUN in ${RUNS[@]}; do
	...
	echo "v2dl3 -ed --full-enclosure -f $ODIR/$RUN.anasum.root $VERITAS_EVNDISP_AUX_DIR/EffectiveAreas/$EFFAREARUN $ODIR/$RUN.anasum.fits" >> $V2DL3SCRIPT
