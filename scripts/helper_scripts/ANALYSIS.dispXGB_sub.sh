...
if [[ -n "${EVNDISP_APPTAINER:-}" ]]; then
    ...
    APPTAINER_MOUNT+=" --bind ${VERITAS_PREPROCESSED_DATA_DIR}:${VERITAS_PREPROCESSED_DATA_DIR} "
    APPTAINER_MOUNT+=" --bind ${ODIR}:${ODIR} "
    APPTAINER_MOUNT+=" --bind ${TEMPDIR}:${TEMPDIR}"
    echo "APPTAINER MOUNT: ${APPTAINER_MOUNT}"
    APPTAINER_ENV="--env VERITAS_EVNDISP_AUX_DIR=${VERITAS_EVNDISP_AUX_DIR},VERITAS_USER_DATA_DIR=${VERITAS_USER_DATA_DIR},VERITAS_PREPROCESSED_DATA_DIR=${VERITAS_PREPROCESSED_DATA_DIR},ODIR=${ODIR},TMPDIR=${TEMPDIR},PYTHONPYCACHEPREFIX=${PYTHONPYCACHEPREFIX}"
