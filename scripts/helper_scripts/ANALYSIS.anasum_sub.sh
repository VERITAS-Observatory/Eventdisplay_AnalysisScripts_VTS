...
prepare_atmo_string()
{
    ...
    # V4 and V5 use GrISU IRFs with atmosphere IDs 21/22.
    if [[ $EPOCH == *"V4"* ]] || [[ $EPOCH == *"V5"* ]]; then
        ATMO=${ATMO/6/2}
    fi
    # V6 UV IRFs are available only for ATM61.
    ...
    if [[ $OBSL == "obsLowHV" ]] || [[ $OBSL == "obsFilter" ]]; then
        if [[ $EFFAREARUN == *"SuperSoft"* ]]; then
            ...
            if [[ $OBSL == "obsLowHV" ]]; then
                ...
                # hard cuts are replaced by moderate cuts!
                EFFAREARUN=${EFFAREARUN/Hard-TMVA-BDT/Moderate}
                RADACCRUN=${RADACCRUN/Hard-TMVA-BDT/Moderate}
                CUTFILE=${CUTFILE/Hard-TMVA-BDT/Moderate}
            else
