...
count_files()
{
    LL=0
    if [[ $1 == "V6" ]]; then
        LL=$(find ${2} -name "*${3}" -exec basename {} "$3" \; | awk -F '-' '{if ($1 > 63372) print $0}' | wc -l) 
    elif [[ $1 == "V4" ]]; then
        LL=$(find ${2} -name "*${3}" -exec basename {} "$3" \; | awk -F '-' '{if ($1 < 46642) print $0}' | wc -l) 
    elif [[ $1 == "V5" ]]; then
        LL=$(find ${2} -name "*${3}" -exec basename {} "$3" \; | awk -F '-' '{if ($1 > 46641 && $1 < 63373) print $0}' | wc -l) 
    fi
    echo "$LL"
}
