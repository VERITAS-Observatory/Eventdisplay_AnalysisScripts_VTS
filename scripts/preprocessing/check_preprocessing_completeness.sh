...
{
    cat <<'EOF'
Usage: check_preprocessing_completeness.sh <production-directory> [report-directory]

Check every baseline <run>.root file against the standard evndisp, mscw, anasum,
and DL3 products. Reports are written to the optional report directory.
EOF
}
...
if [[ "$expected_count" -eq 0 ]]; then
    echo "Error: no baseline files matching <run>.root found below '$ROOT'" >&2
    exit 2
fi
