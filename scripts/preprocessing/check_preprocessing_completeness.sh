...
REFERENCE_RUNS="$REPORT/reference-runs.txt"
if ! cp "$EXPECTED_RUNS" "$REFERENCE_RUNS"; then
    echo "Error: cannot write reference run list '$REFERENCE_RUNS'" >&2
    exit 2
fi
...
reference-runs.txt contains the expected reference runs after applying the
optional exclusion run-list. summary.tsv columns are target, status, expected,
present, missing, unexpected, and duplicate_runs. Missing and unexpected files
contain one run number per line.
